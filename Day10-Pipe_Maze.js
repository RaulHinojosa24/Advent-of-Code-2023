const DATA = `F7-FJ77FL|-L.7.FJ77F-7FF|7-F7F|--F7F-F7.-F---7L7FL7L77-7.F-7F-7FF.--.777F|.7.77-F-7F-LJ7F7F77.F.FF-7|-J-F|F--J--7J7.7--7-LF-J7J7.7.-L-FF77.|
...
function getNextTile(prevX, prevY, currentX, currentY, direction, maze) {
  if (direction === "start") {
    return getStartingTile(currentX, currentY, maze)
  }
  ...
  const newDirection = maze[newY][newX]
...
function getStartingTile(x, y, maze) {
  let newX = x
  let newY = y
  if (["|", "7", "F"].includes(maze[y - 1]?.[x])) newY--
  else if (["|", "J", "L"].includes(maze[y + 1]?.[x])) newY++
  else if (["-", "L", "F"].includes(maze[y]?.[x - 1])) newX--
  else if (["-", "J", "7"].includes(maze[y]?.[x + 1])) newX++
  return {
    x: newX,
    y: newY,
    direction: maze[newY][newX]
  }
}
...
function pipeMaze(data) {
  ...
  startPoint.x = maze[startPoint.y].indexOf("S")

  const steps = [startPoint]
  
  while (steps[steps.length - 1].direction !== "S") {
    const { x: prevX, y: prevY} = steps[steps.length - 2] || {}
    const { x: currentX, y: currentY, direction } = steps[steps.length - 1]
