...
const DATA = `.......358..........31.....339.....669.............598......328.....575......................447..650..............964...........692........
...
function checkIfPartNumber(x, y, data) {
  if (
    (!/\d|\./.test(data[y - 1]?.[x - 1]) && data[y - 1]?.[x - 1] !== undefined) ||
    (!/\d|\./.test(data[y - 1]?.[x]) && data[y - 1]?.[x] !== undefined) ||
    (!/\d|\./.test(data[y - 1]?.[x + 1]) && data[y - 1]?.[x + 1] !== undefined) ||
    (!/\d|\./.test(data[y]?.[x - 1]) && data[y]?.[x - 1] !== undefined) ||
    (!/\d|\./.test(data[y]?.[x + 1]) && data[y]?.[x + 1] !== undefined) ||
    (!/\d|\./.test(data[y + 1]?.[x - 1]) && data[y + 1]?.[x - 1] !== undefined) ||
    (!/\d|\./.test(data[y + 1]?.[x]) && data[y + 1]?.[x] !== undefined) ||
    (!/\d|\./.test(data[y + 1]?.[x + 1]) && data[y + 1]?.[x + 1] !== undefined)
...
function gearRatios(data) {
  ...
  for (let y = 0; y < splittedData.length; y++) {
    for (let x = 0; x < splittedData[y].length; x++) {
      const current = splittedData[y][x]
      ...
      if (!isValid) isValid = checkIfPartNumber(x, y, splittedData)
    }
    if (isValid) {
      validNumbers.push(+number)
    }
...
function getGears(x, y, data) {
  const gears = []

  if (data[y - 1]?.[x - 1] === "*") gears.push(`${x - 1}${y - 1}`)
  if (data[y - 1]?.[x] === "*") gears.push(`${x}${y - 1}`)
  if (data[y - 1]?.[x + 1] === "*") gears.push(`${x + 1}${y - 1}`)
  if (data[y]?.[x - 1] === "*") gears.push(`${x - 1}${y}`)
  if (data[y]?.[x + 1] === "*") gears.push(`${x + 1}${y}`)
  if (data[y + 1]?.[x - 1] === "*") gears.push(`${x - 1}${y + 1}`)
  if (data[y + 1]?.[x] === "*") gears.push(`${x}${y + 1}`)
  if (data[y + 1]?.[x + 1] === "*") gears.push(`${x + 1}${y + 1}`)
...
function gearRatios2(data) {
  ...
  for (let y = 0; y < splittedData.length; y++) {
    for (let x = 0; x < splittedData[y].length; x++) {
      ...
      const newGears = getGears(x, y, splittedData)
      number += current
      gears = gears.concat(newGears)
    }
    if (number) {
      numbers.push({
        number: +number,
  ...
  for (let i = 0; i < numbers.length; i++) {
    ...
    for (let j = 0; j < curr.gears.length; j++) {
      const gear = curr.gears[j]
      if (reduced[gear]) {
        reduced[gear].numbers.push(curr.number)
      } else {
        reduced[gear] = {
          numbers: [curr.number]
        }
  ...
  for (const key in reduced) {
    if (Object.hasOwnProperty.call(reduced, key)) {
      const numbers = reduced[key].numbers;
      
      if (numbers.length === 2) response += numbers[0] * numbers[1]
    }
  }
